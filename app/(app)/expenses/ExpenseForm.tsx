...
export default function ExpenseForm() {
  ...
  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
        run(() => saveExpense({ amountCents: amountC, category, responsibility, paidBy }));
      }}
      className="space-y-4"
    >
      ...
      <label className="block text-sm font-medium text-gray-700">
        Category
        <select className="mt-1 w-full rounded-lg border bg-white p-3 text-lg" value={category} onChange={(e) => setCategory(e.target.value)}>
      ...
      <fieldset>
        ...
        <div className="mt-1 grid grid-cols-3 gap-2">
          {(["nilou", "household", "mom"] as Responsibility[]).map((r) => (
            <button key={r} type="button" onClick={() => setResponsibility(r)}
              className={`rounded-lg p-3 text-sm font-medium capitalize ${responsibility === r ? "bg-brand text-brand-fg" : "bg-gray-100 text-gray-600"}`}>{r}</button>
          ))}
        </div>
      </fieldset>
      <fieldset>
        ...
        <div className="mt-1 grid grid-cols-2 gap-2">
          {(["nilou", "mom"] as const).map((p) => (
            <button key={p} type="button" onClick={() => setPaidBy(p)}
              className={`rounded-lg p-3 text-sm font-medium capitalize ${paidBy === p ? "bg-brand text-brand-fg" : "bg-gray-100 text-gray-600"}`}>{p}</button>
          ))}
        </div>
      </fieldset>
      <div className="rounded-xl bg-gray-900 px-4 py-3 text-white">
        ...
        {responsibility === "household" && (<p className="mt-1 text-xs opacity-70">Household default split: Mom 50%.</p>)}
      </div>
