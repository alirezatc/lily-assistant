...
export default async function SettlementsPage() {
  ...
  if (userId && db) {
    configured = true;
    const exps = await db.select().from(schema.expenses).where(eq(schema.expenses.ownerId, userId));
    const reps = await db.select().from(schema.repayments).where(eq(schema.repayments.ownerId, userId));
      ...
      <section className="rounded-2xl bg-brand p-5 text-brand-fg">
        ...
        {!configured && (
          <p className="mt-2 text-xs opacity-90">
            Connect the database to see live numbers.
          </p>
        )}
      </section>
